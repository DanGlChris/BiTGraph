...
class nconv(nn.Module):
    def __init__(self,inputsize,num_nodes,input_len,pred_len):
        ...
        self.alpha1 = nn.Parameter(torch.zeros(num_nodes, num_nodes)).cpu()
        self.alpha2 = nn.Parameter(torch.zeros(num_nodes, num_nodes)).cpu()


        self.getweight = getweight(self.input_len)
        self.triu_matrix = torch.from_numpy(np.triu(np.ones((self.num_nodes, self.num_nodes)), 1)).cpu().float()
        self.tril_matrix = torch.from_numpy(np.tril(np.ones((self.num_nodes, self.num_nodes)), 1)).cpu().float()
...
class mixprop(nn.Module):
    ...
    def forward(self,x,adj,mask,k,flag=0):
        ...
        adj = adj + torch.eye(adj.size(0)).cpu()
        d = adj.sum(1)
        h = x
        out = [h]
...
class graph_global(nn.Module):
    def __init__(self, nnodes, k, dim, device, alpha=3, static_feat=None):
        ...
        self.nnodes = nnodes
        self.A = nn.Parameter(torch.randn(nnodes, nnodes).cpu(), requires_grad=True).cpu()#to(device)
        ##to(device)
    def forward(self, idx):
...
class graph_undirected(nn.Module):
    ...
    def forward(self, idx):
        ...
        adj = F.relu(torch.tanh(self.alpha*a))
        mask = torch.zeros(idx.size(0), idx.size(0)).cpu()#to(self.device)
        mask.fill_(float('0'))
